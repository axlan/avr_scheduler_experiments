...
uint8_t USART_Send(const uint8_t* data, uint8_t len) {
	...
	for (uint8_t i = 0; i < len; i++) {
		if (!RingBufferPush(data[i], &serial_tx_head, serial_tx_tail, (uint8_t*)serial_tx_buffer, TX_BUFFER_LEN)) {
			break;
		}
		ret++;
	}
...
uint8_t USART_Read(uint8_t task_idx, uint8_t* data, uint8_t len) {
	...
	for (uint8_t i = 0; i < len; i++) {
		if(!RingBufferPop(data + i, serial_rx_head, serial_rx_tail + task_idx, (uint8_t*)serial_rx_buffer, RX_BUFFER_LEN)) {
			break;
		}
		ret++;
	}
